...
class MaterialsTrainer(pl.LightningModule):
    ...
    def compute_loss(self, batch, split_name="val"):
        ...
        target = batch.target.to(output.device)
        l1_loss = nn.L1Loss()
    ...
    def layers_freeze(self, mode='all'):
        ...
        For LEFTNet variants, applies specific logic.
        For other models, checks if the model is CrystalGraphConvNet and applies default logic.
        ...
        """
        # Get the model's class name as a lowercase string
        model_name = self.model.__class__.__name__.lower()

        # Check if the model is a LEFTNet variant
        if "leftnet" in model_name:
            print(f"Model detected as LEFTNet variant: {self.model.__class__.__name__}")

            # LEFTNet-specific freezing logic
            if mode == 'all':
        ...
        # Check if the model is CrystalGraphConvNet for default logic
        ...
        # Raise error for invalid models
            ...
            raise ValueError(f"Invalid model detected: {self.model.__class__.__name__}. "
                            f"Expected models are LEFTNet variants or CrystalGraphConvNet.")
